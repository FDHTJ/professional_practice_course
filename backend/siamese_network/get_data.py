...
for blood,fs in blood2fatures.items():
    ...
    for index_f1 in range(len(fs)):
        ...
        for index_f2 in range(index_f1+1,len(fs)):
            f2=fs[index_f2]
            if tuple(sorted([f1,f2])) in all_tuple:
                continue
            else:
                all_tuple.add(tuple(sorted([f1,f2])))
                blood2generated_data[blood].append({"pigeon1":f1,"pigeon2":f2,"label":1})
            ...
            while negative==None  :
                ...
                if all_fatures[index] not in avoid:
                    ...
                    if tuple(sorted([f1, negative])) in all_tuple:
                        negative=None
            all_tuple.add(tuple(sorted([f1, negative])))
            blood2generated_data[blood].append({"pigeon1": f1, "pigeon2": negative, "label": 0})
...
def get_dataset(all_data,bloods,split_type):
    ...
    with open(f"{split_type}.json","w") as f:
